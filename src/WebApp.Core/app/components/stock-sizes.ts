...
export class StockSizesViewModel extends ViewModelBase {
    ...
    currentComponent = ko.observable<string>().publishOn("currentComponent", true);
    ...
    calculateInStock() {
        ...
        return selectedSize != null && selectedSize.stock.find((item) => { return item.storeid === currentStore.id }) == null;
    }
    ...
    clickOrder = (store: StoreStockViewModel) => {
        if(store instanceof StoreStockViewModel){
            const order = this.repo.createOrder({id: store.storeid, name: store.storename}, this.product(), this.selectedSize().size);
            this.orders.unshift(order);
        } else {
            const order = this.repo.createOrder(this.currentStore(), this.product(), this.selectedSize().size);
            this.orders.unshift(order);
        }
